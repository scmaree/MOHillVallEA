...
void hillvallea::optimizer_t::initialize(elitist_archive_t & elitist_archive, unsigned int & number_of_evaluations)
{
  // creat a new population
  population = std::make_shared<population_t>();
  if (fitness_function->redefine_random_initialization) {
    fitness_function->init_solutions_randomly(population, population_size, 0, rng);
  }
  else
  {
...
void hillvallea::optimizer_t::generation(elitist_archive_t & elitist_archive, unsigned int & number_of_evaluations)
{
  ...
  for(size_t i = 0; i < subpopulations.size(); ++i)
  {
    ...
    //----------------------------------------------------------------------
    // Improvement?: handle merged elitist archives.
    std::vector<cluster_pt> previous_clusters;
    for(int j = 0; j < previous_subpopulations.size(); ++j)
    {
...
// initializeNewPopulation( void )
void hillvallea::optimizer_t::initialize_mm(elitist_archive_t & elitist_archive, unsigned int & number_of_evaluations)
{
  ...
  if (fitness_function->redefine_random_initialization) {
    fitness_function->init_solutions_randomly(population, number_of_elites_added + population_size, number_of_elites_added, rng);
  }
  else
  {
  ...
  subpopulations.clear();
  bool add_test_sols = true, recheck_elites = true;
...
void hillvallea::optimizer_t::generation_mm(elitist_archive_t & elitist_archive, unsigned int & number_of_evaluations, bool largest_population)
{
...
void hillvallea::optimizer_t::linkSubpopulations(std::vector<population_pt> & subpopulations, std::vector<population_pt> & previous_subpopulations, const std::vector<vec_t> & previous_means ) const
{
...
void hillvallea::optimizer_t::generateOffspring(population_t & pop, size_t number_of_solutions)
{
...
void hillvallea::optimizer_t::removeElitesFromSubpopulations(size_t max_elites_per_subpop, std::vector<population_pt> & subpopulations, rng_pt & rng) const
{
...
void hillvallea::optimizer_t::updateStrategyParameters(const population_t & population, std::vector<cluster_pt> & clusters, const elitist_archive_t & previous_elitist_archive, size_t & no_improvement_stretch, const size_t maximum_no_improvement_stretch, const vec_t & objective_ranges) const
{
...
void hillvallea::optimizer_t::generateAndEvaluateNewSolutionsToFillPopulationNoElites(population_t & population, size_t number_of_solutions_to_generate, const std::vector<cluster_pt> & clusters, unsigned int & number_of_evaluations, rng_pt & rng) const
{
...
void hillvallea::optimizer_t::generateAndEvaluateNewSolutionsToFillPopulation(population_t & population, size_t number_of_solutions_to_generate, const std::vector<cluster_pt> & clusters, size_t & number_of_elites, unsigned int & number_of_evaluations, rng_pt & rng) const
{
...
void hillvallea::optimizer_t::mamalgam_bugfix_clustering(const population_t & pop, size_t selection_size, std::vector<cluster_pt> & clusters, size_t number_of_clusters, size_t cluster_size, double average_edge_length, rng_pt & rng) const
{
  // add MO clusters
...
void hillvallea::optimizer_t::mamalgam_clustering(const population_t & pop, std::vector<cluster_pt> & clusters, size_t number_of_clusters, size_t cluster_size, size_t selection_size, rng_pt & rng) const
{
  clusters.clear();
...
void hillvallea::optimizer_t::mamalgam_MO_clustering(const population_t & pop, std::vector<cluster_pt> & clusters, size_t number_of_clusters, size_t cluster_size, size_t selection_size, rng_pt & rng) const
{
...
void hillvallea::optimizer_t::mamalgam_SO_clustering(const population_t & pop, std::vector<cluster_pt> & clusters, size_t cluster_size) const
{
  ...
  for (size_t i = 0; i < number_of_objectives; i++)
  {
    ...
    {
      if (pop.sols[j]->penalty == 0) {
        individual_objectives[j] = pop.sols[j]->obj[i];
      }
      else
      {
        individual_objectives[i] = pop.worst_objective[i] + pop.sols[j]->penalty;
      }
    }
...
void hillvallea::optimizer_t::mamalgam_cluster_registration(std::vector<cluster_pt> & clusters, const std::vector<cluster_pt> & previous_clusters, const vec_t & obj_ranges)
{

  int number_of_nearest_neighbours_in_registration = 7;
...
void hillvallea::optimizer_t::direct_cluster_registration_on_mean(std::vector<cluster_pt> & clusters, const std::vector<cluster_pt> & previous_clusters, const vec_t & obj_ranges) const
{
